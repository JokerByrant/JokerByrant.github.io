...
## 后记
补充于 `2021-12-2`：这篇文章中分析的 `Spring` 容器的 `refresh` 操作其实就是 `Spring IOC` 容器的初始化过程，上面只是对整个流程进行了粗略的分析，其中一些关键的细节并没有深入进去，包括 [创建BeanFactory,加载并注册Bean](#2-obtainFreshBeanFactory)、[初始化所有的singleton beans](#11-finishBeanFactoryInitialization-beanFactory)。这两处代码还是有必要具体看一下的，具体可以看一下这篇博客：[Spring IOC 容器源码分析](https://javadoop.com/post/spring-ioc#toc_7)，大佬讲的很详细。
